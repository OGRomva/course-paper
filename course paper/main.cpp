...
int main() {
	...
	std::getline(std::cin, strWithExpr);
 	...
 	while (i < strWithExpr.size()) {
		...
		if ((countParenthesis > 0) && (i == strWithExpr.size() - 1) && (strWithExpr[i] != ')')) {
		...
		if (strWithExpr[i] == ')' && countParenthesis != 0) {
		...
		} else if (strWithExpr[i] == ')' && countParenthesis == 0) {
